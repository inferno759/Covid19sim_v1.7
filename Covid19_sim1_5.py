...
totalPeoplePop = 100    # total population
...
def userInputs():
    ...
    debugMode = \
        (input("\nif using debug mode enter '1' ") or "0")
    totalPeoplePop = \
        int(input("Insert the total population for the simulation, default 100 ") or "100")
    totalDays = \
...
def main():
    ...
    createSafePeople(totalPeoplePop, safePeoplePop, 0, defaultActive, infectionChance, deathChance)
    createUnsafePeople(totalPeoplePop, safePeoplePop, 0, defaultActive, infectionChance, deathChance)
    createRiskFactor(totalPeoplePop, riskFactorPop)
    infectionDeathChanceGen(deathChance)
    createInitialInfected(initialInfected)
    simulateTime(debugMode, totalDays, totalDeathList)
...
def createEncounter(debugMode, atHomeTick):  #encounterZone
    global personList
    global totalInfected
    ...
    for person in range(personListEnd - atHomeTick):        # only counts the population still active, not at Home
        if person == 0:                                     # used for beginning index where only right side can infect
            ...
            personInfect = getattr(personList[person], 'infectChance')
            if debugMode == 1:                              # used for middle indexes where left and right can infect
                print("\n encounter tick: ", person)
...
def createDay(debugMode):
    ...
    while atHomeTick < len(personList) - 1:
        ...
        createEncounter(debugMode, atHomeTick)
...
def simulateTime(debugMode, totalDays, totalDeathList):
    global personList
    global deathChance
    ...
    for day in range(totalDays):                    # run for total amount of days
        ...
        createDay(debugMode)                                            # create new day, which creates new encounters
        dayCount += 1
        print("Day: ", dayCount)
...
def createSafePeople(totalPeoplePop, safePeoplePop, riskFactor, defaultActive, infectChance, deathChance):
    global personList
...
def createUnsafePeople(totalPeoplePop, safePeoplePop, riskFactor, defaultActive, infectChance, deathChance):
    global personList
...
def createRiskFactor(totalPeoplePop, riskFactorPop):
    global personList
    riskFactorTick = 0
...
def createInitialInfected(initialInfected):
    global personList
...
def infectionDeathChanceGen(deathChance):
    global personList
